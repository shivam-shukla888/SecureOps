...
export const CredentialsView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 select-none">
      ...
      {oneTimeSecretData && (
        <div className="p-5 rounded-xl bg-[#0f172a] border-2 border-emerald-500/60 shadow-xl space-y-4 font-mono text-xs">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 p-3 rounded-lg bg-[#0a0f1b] border border-cyan-500/40 text-cyan-300 font-bold font-mono text-xs select-all break-all">
                {oneTimeSecretData.api_key}
              </div>
              <button
                onClick={copySecret}
                className="px-4 py-3 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-semibold flex items-center gap-1.5 transition-colors shrink-0 cursor-pointer"
              >
      ...
      {revokingCred && (
        <div className="p-5 rounded-xl bg-[#1e1014] border-2 border-rose-500/60 shadow-xl space-y-4 font-mono text-xs">
          ...
          <p className="text-slate-300 text-[11px] font-sans leading-relaxed">
            Are you sure you want to revoke credential <strong className="text-white font-mono">{revokingCred.name}</strong> ({revokingCred.credential_id})? Existing AI agents or services using this credential will immediately lose access to the SecureOps gateway.
          </p>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              ...
              disabled={actionLoading}
              className="px-4 py-1.5 rounded-lg bg-rose-600 hover:bg-rose-500 text-white font-semibold text-xs flex items-center gap-1.5 transition-colors disabled:opacity-50 cursor-pointer"
            >
      ...
      {/* Main Grid: Generator/Filters on Left (5 cols) + Credential List/Details on Right (7 cols) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="pt-2 border-t border-slate-800 flex justify-end">
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-semibold text-xs flex items-center gap-1.5 transition-colors disabled:opacity-50 cursor-pointer"
                >
                  {loading ? (
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="rounded-xl bg-[#111827] border border-slate-800/80 overflow-hidden font-mono text-xs">
                  ...
                  <tbody className="divide-y divide-slate-800/60">
                    {filteredCredentials.map((cred) => {
                      ...
                      return (
                        ...
                        >
                          <td className="p-3">
                            <span className="text-slate-200 font-bold block">{cred.name}</span>
                            <span className="text-[10px] text-cyan-400 block select-all">
                              {cred.credential_id}
                            </span>
                          </td>
                          <td className="p-3 text-slate-300">{cred.user_id}</td>
                          <td className="p-3">
            ...
            <div className="p-5 rounded-xl bg-[#111827] border border-slate-800/80 space-y-3 font-mono text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-[#0e1422] border border-slate-800 space-y-1">
                  ...
                  <strong className="text-slate-200 block truncate">{selectedCred.name}</strong>
                </div>
                <div className="p-3 rounded-lg bg-[#0e1422] border border-slate-800 space-y-1">
                  <span className="text-[10px] text-slate-400 block">USER</span>
                  <strong className="text-slate-200 block truncate">{selectedCred.user_id}</strong>
                </div>
                <div className="p-3 rounded-lg bg-[#0e1422] border border-slate-800 space-y-1">
              ...
              {canManage && selectedCred.status === 'ACTIVE' && (
                <div className="pt-2 border-t border-slate-800 flex justify-end gap-3">
                  <button
                    ...
                    disabled={actionLoading}
                    className="px-3 py-1.5 rounded-lg bg-cyan-600/20 hover:bg-cyan-600/30 text-cyan-300 border border-cyan-500/40 text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    disabled={actionLoading}
                    className="px-3 py-1.5 rounded-lg bg-rose-600/20 hover:bg-rose-600/30 text-rose-300 border border-rose-500/40 text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                    <span>Revoke</span>
                  </button>
