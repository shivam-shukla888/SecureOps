...
export const ExecutionCenterView: React.FC = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 select-none max-w-full">
      ...
      {/* Main 2-Column Grid: Dispatcher on Left (5 cols on lg), Result/Trace on Right (7 cols on lg) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 items-start">
        ...
        <div className="lg:col-span-5 space-y-4 w-full">
          <div className="p-4 sm:p-5 rounded-xl bg-[#111827] border border-slate-800/80 space-y-4">
            ...
            <form onSubmit={handleExecute} className="space-y-3.5 sm:space-y-4 font-mono text-xs">
              <div>
              ...
              {(toolName === 'update_data_tool' || toolName === 'delete_data_tool') && (
                <div>
                  <label className="block text-amber-400 mb-1.5 font-medium flex items-center justify-between gap-2 flex-wrap">
                  ...
                  <input
                    ...
                    disabled={loading}
                    className="w-full bg-[#0a0f1b] border border-amber-500/40 focus:border-amber-400 rounded-lg py-2.5 sm:py-2 px-3 text-xs text-amber-200 focus:outline-none disabled:opacity-50 min-h-[44px] sm:min-h-0"
                  />
                </div>
              )}
              ...
              <div className="pt-1">
                ...
                <div className="space-y-1.5">
                  <button
                    ...
                    className="w-full text-left p-2.5 sm:p-2 min-h-[44px] sm:min-h-0 rounded-lg bg-[#0e1422] border border-slate-800 hover:border-emerald-500/40 text-[11px] transition-colors flex items-center justify-between group cursor-pointer gap-2"
                  >
                    <span className="text-slate-300 group-hover:text-emerald-300 truncate">
                      Safe Document Search
                    </span>
                    <span className="text-[10px] text-emerald-400 uppercase shrink-0 font-bold">DIRECT DISPATCH</span>
                  </button>
                  ...
                  <button
                    ...
                    className="w-full text-left p-2.5 sm:p-2 min-h-[44px] sm:min-h-0 rounded-lg bg-[#0e1422] border border-slate-800 hover:border-cyan-500/40 text-[11px] transition-colors flex items-center justify-between group cursor-pointer gap-2"
                  >
                    <span className="text-slate-300 group-hover:text-cyan-300 truncate">
                      Safe Metrics Read
                    </span>
                    <span className="text-[10px] text-cyan-400 uppercase shrink-0 font-bold">DIRECT DISPATCH</span>
                  </button>
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-slate-800 flex items-center justify-between gap-2.5">
                <button
                  ...
                  className="px-3 py-2 min-h-[44px] sm:min-h-0 rounded-lg bg-slate-800/60 hover:bg-slate-800 text-slate-400 hover:text-slate-200 text-xs transition-colors flex items-center justify-center gap-1.5 disabled:opacity-40 cursor-pointer"
                >
                ...
                <button
                  ...
                  className="px-4 py-2 min-h-[44px] sm:min-h-0 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-xs flex items-center justify-center gap-2 transition-colors disabled:opacity-50 cursor-pointer"
                >
                  {loading ? (
                    <>
        ...
        <div className="lg:col-span-7 space-y-4 w-full">
          ...
          {/* Loading State */}
          {loading && (
            <div className="p-5 sm:p-6 rounded-xl bg-[#111827] border border-slate-800/80 space-y-4 font-mono">
          ...
          {result && (
            <div className="space-y-4 w-full">
              ...
              <div
                className={`p-4 sm:p-5 rounded-xl border flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4 font-mono ${
              ...
              >
                ...
                <div className="md:text-right text-xs shrink-0 pt-2 md:pt-0 border-t md:border-t-0 border-slate-800">
                  <span className="text-slate-400 block text-[10px]">EXECUTION ID</span>
                  <span className="font-bold text-white select-all break-all">{result.execution_id}</span>
                </div>
              </div>
              ...
              <div className="p-4 sm:p-5 rounded-xl bg-[#111827] border border-slate-800/80 space-y-3 font-mono text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
                  <div className="p-3 rounded-lg bg-[#0e1422] border border-slate-800 space-y-1">
                    ...
                    <strong className="text-slate-200 block truncate select-all break-all">
                      {result.request_id}
                    </strong>
                  </div>
                  <div className="p-3 rounded-lg bg-[#0e1422] border border-slate-800 space-y-1">
              ...
              {/* Result Payload JSON */}
              <div className="p-4 sm:p-5 rounded-xl bg-[#111827] border border-slate-800/80 space-y-2 font-mono text-xs max-w-full overflow-hidden">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  <div className="flex items-center gap-2">
                    <Cpu className="w-4 h-4 text-cyan-400 shrink-0" />
                    <h4 className="text-xs font-semibold text-white uppercase">
                ...
                <pre className="p-3 rounded-lg bg-[#0a0f1b] border border-slate-800 text-[11px] text-cyan-300 overflow-x-auto whitespace-pre max-w-full">
              ...
              <div className="space-y-1.5">
                {sessionHistory.map((item, idx) => (
                  <div
                    ...
                    className="p-2.5 rounded-lg bg-[#0e1422] border border-slate-800 hover:border-cyan-500/40 cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-2 transition-colors min-h-[44px]"
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-cyan-400 font-bold shrink-0">{item.tool_name}</span>
                      <span className="text-slate-500 text-[10px] select-all truncate break-all">
                        {item.execution_id}
                      </span>
                    </div>
